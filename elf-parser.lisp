...
(defun read-elf-header (in)
  (let ((header (make-instance 'elf-header)) endianess bits)
    (with-slots (EI_MAG0 EI_CLASS EI_DATA EI_VERSION EI_OSABI
      ...
      (setf e_type (read-ubytes 2 in endianess))
      (setf e_machine (read-ubytes 2 in endianess))
      (setf e_version (read-ubytes 4 in endianess))
      (setf e_entry (if bits
			(read-ubytes 4 in endianess)
			(read-ubytes 8 in endianess)))
      ...
      (setf e_shoff (if bits
			(read-ubytes 4 in endianess)
			(read-ubytes 8 in endianess)))
      (setf e_flags (read-ubytes 4 in endianess))
      (setf e_ehsize (read-ubytes 2 in endianess))
      (setf e_phentsize (read-ubytes 2 in endianess))
      (setf e_phnum (read-ubytes 2 in endianess))
      (setf e_shentsize (read-ubytes 2 in endianess))
      (setf e_shnum (read-ubytes 2 in endianess))
      (setf e_shstrndx (read-ubytes 2 in endianess)))
    header))
